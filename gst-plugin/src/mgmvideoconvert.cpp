...
static gboolean gst_magma_videoconvert_set_caps(GstBaseTransform* trans, GstCaps* incaps, GstCaps* outcaps) {
    ...
    gboolean in_dmabuf = gst_caps_features_contains(gst_caps_get_features(incaps, 0), GST_CAPS_FEATURE_MEMORY_DMABUF);
    gboolean out_dmabuf = gst_caps_features_contains(gst_caps_get_features(outcaps, 0), GST_CAPS_FEATURE_MEMORY_DMABUF);
    self->need_upload = !in_dmabuf && out_dmabuf;
    self->need_download = in_dmabuf && !out_dmabuf;

    GST_INFO_OBJECT(self, "Input %dx%d stride=%d%s%s", self->in_width, self->in_height, self->in_stride, self->need_upload ? " UPLOAD" : "", self->need_download ? " DOWNLOAD" : "");
...
static GstFlowReturn gst_magma_videoconvert_transform(GstBaseTransform* trans, GstBuffer* inbuf, GstBuffer* outbuf) {
    ...
    if (self->need_upload) {
        ...
        if (stride == (gint)pitch) {
            hipError_t err = hipMemcpyAsync(self->d_image, in_map.data, self->gpu_size, hipMemcpyHostToDevice, self->hip_stream);
            if (err != hipSuccess) {
            ...
            for (gint y = 0; y < h; y++) {
                hipError_t err = hipMemcpyAsync((guint8*)self->d_image + y * pitch, in_map.data + y * stride, (gsize)w, hipMemcpyHostToDevice, self->hip_stream);
                if (err != hipSuccess) {
            ...
            for (gint y = 0; y < h / 2; y++) {
                hipError_t err = hipMemcpyAsync((guint8*)self->d_image + pitch * h + y * pitch, src_uv + y * stride, (gsize)w, hipMemcpyHostToDevice, self->hip_stream);
                if (err != hipSuccess) {
        ...
        if (dst_stride == (gint)src_stride) {
            ...
            for (gint y = 0; y < h; y++) {
                err = hipMemcpy(out_map.data + y * dst_stride, (guint8*)d_ptr + y * src_stride, (gsize)w, hipMemcpyDeviceToHost);
                if (err != hipSuccess)
                    break;
            }
            if (err == hipSuccess) {
                ...
                for (gint y = 0; y < h / 2; y++) {
                    err = hipMemcpy(out_map.data + dst_stride * h + y * dst_stride, src_uv + y * src_stride, (gsize)w, hipMemcpyDeviceToHost);
                    if (err != hipSuccess)
                        break;
                }
            }
