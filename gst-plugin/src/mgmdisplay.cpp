...
static int fb_from_dmabuf(GstMagmaDisplay* self, int dmabuf_fd, int fb_w, int fb_h) {
    ...
    uint32_t stride = (uint32_t)self->gbm_stride;
    if (!stride) stride = self->in_width; // fallback if unset (Path 1)
...
static GstFlowReturn gbm_bo_to_display(GstMagmaDisplay* self) {
    ...
    int fb_id = fb_from_dmabuf(self, bo_fd, self->mode.hdisplay, self->mode.vdisplay);
    close(bo_fd);
...
static GstFlowReturn gst_magma_display_render(GstBaseSink* sink, GstBuffer* buf) {
    ...
    // Path 1: DMABuf memory — zero-copy
    ...
    if (mem && gst_is_dmabuf_memory(mem)) {
        ...
        int fb_id = fb_from_dmabuf(self, dup_fd, self->in_width, self->in_height);
        close(dup_fd);
        return display_fb(self, fb_id);
    }
    ...
    if (hmeta) {
        ...
        return copy_nv12_to_gbm(self, (const void*)hmeta->d_ptr, self->in_width, hipMemcpyDeviceToDevice);
    }
    ...
    guint src_stride = vmeta && vmeta->stride[0] > 0 ? (guint)vmeta->stride[0] : self->in_width;
    ...
    GstFlowReturn gret = copy_nv12_to_gbm(self, map.data, src_stride, hipMemcpyHostToDevice);
