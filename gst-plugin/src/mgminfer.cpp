...
MagmaInferObject* magma_infer_object_new(
    guint class_id, const gchar* label, gfloat confidence, gfloat x, gfloat y, gfloat w, gfloat h) {
...
static gboolean magma_inference_meta_transform(
    ...
    dest = magma_buffer_add_inference_meta(transbuf, src->source_width, src->source_height);
    if (!dest)
        return FALSE;
...
const GstMetaInfo* magma_inference_meta_get_info(void) {
    ...
    if (g_once_init_enter(&_magma_inference_meta_info)) {
        const GstMetaInfo* info = gst_meta_register(MAGMA_INFERENCE_META_API_TYPE,
                                                    "MagmaInferenceMeta",
...
MagmaInferenceMeta*
magma_buffer_add_inference_meta(GstBuffer* buffer, guint source_width, guint source_height) {

    MagmaInferenceMeta* m =
        (MagmaInferenceMeta*)gst_buffer_add_meta(buffer, magma_inference_meta_get_info(), NULL);
    if (m) {
...
/** --- PROPERTIES --- */
static void gst_magma_infer_set_property(GObject* object,
                                         guint prop_id,
                                         const GValue* value,
                                         GParamSpec* pspec) {
...
static void
gst_magma_infer_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec) {
    GstMagmaInfer* self = GST_MAGMA_INFER(object);
...
/** --- CAPS NEGOTIATION --- */
static gboolean
gst_magma_infer_set_caps(GstBaseTransform* trans, GstCaps* incaps, GstCaps* outcaps) {
    ...
    GST_INFO_OBJECT(
        self, "configured %dx%d %s", self->in_width, self->in_height, GST_VIDEO_INFO_NAME(&info));
    return TRUE;
}
...
static GstFlowReturn gst_magma_infer_transform_ip(GstBaseTransform* trans, GstBuffer* buf) {
    ...
    if (!magma_buffer_add_inference_meta(buf, self->in_width, self->in_height)) {
...
static void gst_magma_infer_class_init(GstMagmaInferClass* klass) {
    ...
    g_object_class_install_property(
        gobject_class,
        PROP_INFERENCE_INTERVAL,
        g_param_spec_uint("inference-interval",
                          "Inference interval",
