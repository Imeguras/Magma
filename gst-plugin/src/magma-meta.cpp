...
static gboolean magma_tensor_meta_transform(GstBuffer *transbuf, GstMeta *meta,
                                             GstBuffer *buffer, GQuark type, gpointer data) {
    if (type != 0)
        return FALSE;
    MagmaTensorMeta *src = (MagmaTensorMeta *)meta;
    MagmaTensorMeta *dest;
    dest = magma_buffer_add_tensor_meta(transbuf, src->tensor_mem,
...
const GstMetaInfo *magma_tensor_meta_get_info(void) {
    if (g_once_init_enter(&_magma_tensor_meta_info)) {
        const GstMetaInfo *info = gst_meta_register(
            MAGMA_TENSOR_META_API_TYPE, "MagmaTensorMeta",
            sizeof(MagmaTensorMeta),
            magma_tensor_meta_init,
            magma_tensor_meta_free,
            magma_tensor_meta_transform);
